...
export abstract class GenericTypeOrmAdapter<E, O extends ObjectLiteral, PK extends keyof O> {
  ...
  async findOneByIdOrFail(id: O[PK]): Promise<E> {
    ...
    if (!e) {
      throw new Error(`${this.repository.metadata.name} with ${String(this.primaryKeyName)}=${id} not found`);
    }
    return e;
  }
  ...
  async update(id: O[PK], domain: E): Promise<E> {
    ...
    await this.repository.update({ [this.primaryKeyName]: id } as FindOptionsWhere<O>, this.toEntityDB(domain) as any);
